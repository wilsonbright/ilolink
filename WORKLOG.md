...
- **Diagnosed first, and it was not a cache.** Pulled the live assets and hashed them against the repo: byte-identical. The Modernist redesign (`9b2a090`) swapped blue `#3b5bdb` for red `#ec3013` in every *source* file and left the **rasters** untouched — `app/opengraph-image.png` (blue pill + the pre-redesign tagline, exactly the image PH renders), `app/apple-icon.png`, `app/favicon.ico`, plus `docs/launch/assets/{social-card,thumbnail}.{html,png}`. Already correct and left alone: `app/icon.svg`, `app/api/og/route.tsx` (per-document cards), `launch/product-hunt-2026-08-14/assets/thumbnail-240.png`. No web manifest, no email logo, and `content-worker` points every doc at `/api/og`, so nothing else carried the mark.
- **Why it went unseen for two days:** Next emits BOTH icon links, and Chrome/Firefox prefer `icon.svg` (`sizes="any"`) — so the tab looked right locally while Safari, Slack, and every scraper took `favicon.ico` and got blue.
- **Why no test caught it, and why one still can't.** `test/seo-metadata.test.ts` asserts those files exist, are non-empty, and that the OG image is 1200×630 — all of which a stale image satisfies perfectly. There is no assertion that a PNG shows the current logo. Replaced the missing guard with a procedural one: **`docs/brand/`** (new) holds the board sources and regenerates all three rasters via `npm run brand`, and `lib/ui/logo.tsx`'s "restated in three files" comment now names the rasters as the fourth, uncheckable copy.
- **Rendering:** no ImageMagick / rsvg / sharp on this machine, so the boards are HTML/CSS rendered by Playwright's headless Chrome over `localhost` (`file://` blocks subresources — it would silently drop both `_board.css` and the Archivo webfont). Icons come from ONE 512 master downsampled to 180/48/32/16: 512 is 16× the mark's 32-unit grid, so every edge is whole-pixel and each step is an exact integer ratio. `docs/brand/make-ico.mjs` packs the multi-frame `.ico` by hand (PNG-in-ICO) since nothing installed can.
- **Cards restyled into the DS, wording kept.** OG card is now Archivo 800 on canvas with a 2px divider and the mark at 36px — structurally mirroring `api/og` so a site share and a doc share read as one system. The old card's giant "ilolink" wordmark went (the top-left lockup already says it); its tagline became the headline plus the footer line. `opengraph-image.alt.txt` still describes it accurately, so it was left alone.
- **Verified, not inferred:** looked at every regenerated PNG; parsed the `.ico` back apart (3 frames, offsets resolve, each IHDR matches its directory entry); tsc clean; **460/460 tests**; `next build` green; then ran `next start` and **hashed the served bytes against the repo files — all three pair exactly**, with Next's cache-busting hashes changed (`024fd44e…` → `107e4106…`), so unfurlers will refetch rather than serve a cached blue card.
- **Not done, needs a human:** the Product Hunt gallery image and 240px thumbnail are *uploaded copies*, not live fetches — deploying does not update them. Both must be re-uploaded (`launch/product-hunt-2026-08-14/assets/thumbnail-240.png` is already the correct red one and was simply never uploaded).
- **Files:** `app/opengraph-image.png`, `app/apple-icon.png`, `app/favicon.ico`, `lib/ui/logo.tsx`, `package.json`, `docs/launch/assets/social-card.html`, `docs/launch/assets/thumbnail.html`, new `docs/brand/{README.md,_board.css,opengraph-image.html,app-icon.html,make-ico.mjs,render.sh}`. (`docs/launch/assets/*.png` regenerated too, but they are `.gitignore`d — only their HTML sources are tracked.)

## 2026-08-14 — Fixed the two critical file-bomb blockers (audit remediation)

- **Asked:** fix the critical issues, without impacting functionality/UX.
- **Blocker 1 — unbounded R2 accumulation:** two vectors, two fixes.
  - *Docs:* added `pruneSupersededVersionsWith` to `lib/publish/store-core.ts` — deletes every non-current version's R2 bodies + rows after the current-version + KV slug swap. Wired into both store composites (`storeVersionWith`/`storeBinaryVersionWith`, covering the app publish + MCP publish paths) and into `updateDoc` (the MCP update path, which builds versions directly). Bounds every doc to ONE stored version and is self-healing (cleans historical bloat on the next write). Zero UX impact — verified there is no rollback/history consumer: every read goes through `current_version_id`. Best-effort (`.catch(() => {})`) so cleanup never fails a user's publish.
  - *Artifacts:* can't prune (versions are load-bearing: proposals consume version numbers, `if_version` compares against published), so added a per-teamspace ceiling `MAX_VERSIONS_PER_TEAMSPACE = 20_000`, checked in `putArtifact` BEFORE any insert (no orphan artifact row on refuse). Deliberately an order of magnitude above any plausible real registry — converts "unbounded" to "bounded" without touching legitimate use. Plan-aware quota noted as a follow-up product decision.
- **Blocker 2 — docx decompression bomb:** the 15 MB gate checked the still-zipped upload; now the CONVERTED HTML is re-checked against the text ceiling before storing, at all three sites (`mcp-worker/src/docs.ts`, `mcp-worker/src/publish-core.ts`, `app/api/publish/route.ts`).
- **Tests:** +5 (465 total). `test/store-core.test.ts` — prune deletes both R2 bodies of each superseded version then the rows, keeps current, no-ops on a fresh doc. `test/artifact-quota.test.ts` — refuses at the ceiling writing nothing, allows below, counts by the target teamspace.
- **Verified:** 465/465, tsc clean across app+mcp+trends, deployed app `877d1f14` + mcp `5b79941b`, live health probes clean (200/401, no 500s — write paths load + route). **Boundary stated honestly:** a live `update_document` loop watching R2 stay flat was NOT run — needs an authenticated MCP session (PAT) I can't mint without inbox access. Serving unaffected by construction. The 3 lesser findings (proposal flood, webhook ordering, `/_feedback` visibility) left open — not critical.

## 2026-08-14 — Pre-launch security audit (full report: SECURITY-AUDIT-2026-08-14.md)

- **Asked:** full security audit — payment gaps, subscription hijack, agent file-bombing, plan-quota breach, no client-side tokens/keys, anything launch-breaking.
- **Ran a 5-surface parallel audit** (payments, quota/file-bomb, client-secrets, authz, launch-surface), each auditor forced to adversarially disprove its own findings; synthesizer re-judged for launch severity. I then **hand-verified the two blockers line-by-line** against source (didn't take the agents at face value).
- **Verdict: NO-GO** until two accumulating-storage holes are capped — the one class the audit found real. **Auth, payments/subscription, and client-secrets surfaces are clean** (no money-bypass, no cross-tenant IDOR, no leaked secret survived verification; only `NEXT_PUBLIC_TURNSTILE_SITEKEY` — a publishable key — reaches the browser).
- **Blocker 1 (confirmed by hand):** `update_document` (mcp-worker/src/docs.ts) and `artifacts_put`/`artifacts_push` (lib/artifacts/store-core.ts) create new R2 objects that are **never pruned**, gated only by a per-minute rate limit — no per-teamspace byte/count/plan ceiling. A free account loops to hundreds of GB/day of permanent R2. The handler comment at agent.ts:559 admits "the rate limit is the only ceiling."
- **Blocker 2 (confirmed by hand):** docx upload gate checks the *compressed* 15 MB size, then `docxToHtml` inflates it and stores the result **without re-checking against `MAX_TEXT_BYTES`** (docs.ts:227-233, unlike the text branch at :238) — decompression bomb → oversized store or 128 MB worker OOM.
- **Also:** MEDIUM proposal-queue flood (`countProposals` guard only on `artifacts_contribute`, not `_put`/`_push`); LOW webhook marks event processed before the plan grant commits (paid-but-unprovisioned on transient D1 failure); LOW `/_feedback` doesn't gate on private/unpublished. Full exploits + fixes in SECURITY-AUDIT-2026-08-14.md. **No code changed — audit only.**
...
- **Architecture (decided before building):** a fourth worker, `trends-worker/`, with its **own D1** (`ilolink-trends`, own migration chain from `0001_init.sql` — the product chain in `migrations/` never touches it) and the **shared KV** namespace as the one-way hand-off: the worker writes only `trending:*` keys, the app only reads them. Rationale recorded in the wrangler header: a bug here can at worst publish a bad list, never reach product data. Same repo because the SEO archive must share the domain's authority, and phase 4 wants `trending_list` inside the audited MCP server.
- **Keyless by design:** `GITHUB_TOKEN` is optional. Without it, star snapshots ride ON search results (100 repos/page in bulk) instead of per-repo fetches, budgets shrink (3 pages/query, 30 top-ups), requests are spaced for the 10/min search limit, and everything skipped is counted into `source_runs.error` — never silently dropped. Every ingest fetch checks an absolute 13-min deadline so the Sunday cron can't be killed mid-run with `source_runs` unwritten.
- **Honest first week:** velocity needs a prior snapshot, so the first-ever compute **refuses** ("insufficient history") rather than fabricate a spike; real rankings start with the second weekly snapshot. Same doctrine at card level: a repo only counts as born-this-week (prior=0) if created inside the scored week.
- **Publishing is a human act:** `POST /admin/approve` is the only writer of `trending:weeks` / `trending:{week}`; compute writes immutable `trending_snapshots` rows and the page shows nothing until approval. Admin API (workers.dev) fails **closed** — every route 503s if `ADMIN_TOKEN` is unset, 401 otherwise; timing-safe compare.
- **Ultracode run:** 8 agents (recon ×2 → implement ×2 → integrate → review ×2 → fix). Review caught 3 important defects before deploy: the breakout hero compared z-scores against ln-scale scores across kinds (now rank-1 per kind by starVel — the one shared scale); corroboration chips rendered the internal enum `awesome_list` and lost WHICH list corroborated (now joined from `awesome_seen`, named per list); ingest arithmetic exceeded both the 15-min cron cap and the ~100s HTTP proxy timeout (deadline budget added; the reviewer's 202+`waitUntil` shape for `/admin/ingest` then **failed in production too** — see below). Plus: `javascript:` href hardening in the app-side card validator, dot-segment rejection on `/admin/watchlist`, `maxPerKind` clamped so approve stays under D1's 100-bound-param limit, stale-repo starvation guard on the top-up queue.
- **Files:** `trends-worker/` (wrangler, migrations, src: index/github/awesome/classify/scoring/compute/publish/week/types), `lib/trending/` (types + strict readers), `app/(marketing)/trending/` + `[week]` archive, `app/api/trending/`, nav + footer links, sitemap registration (`/trending` only — archive weeks deliberately excluded, discovered via links), 5 new test files.
- **Verified:** tsc clean both projects, 453/453 tests, `next build` green. **Deployed:** migration applied to remote `ilolink-trends` (`82b6936f…`), trends-worker `1bff733b` (crons live: Sun 22:00 ingest, Mon 06:00 compute), `ADMIN_TOKEN` set, app worker `48258b83`. **Live probes:** admin 401 without token / 404 unknown route / status OK with token; `ilolink.com/trending` 200 and the empty state screenshotted — it's a real page (pitch, ranking explainer, cross-links), not ten empty sections; `/api/trending` 404 and archive 404 both correct until first approval.
- **The 202+`waitUntil` manual-ingest shape died in production — caught by watching, not reasoning.** `source_runs` stayed empty through three trigger attempts; `wrangler tail` showed the truth: *"waitUntil() tasks did not complete within the allowed time after invocation end and have been cancelled"* — the runtime grants ~30s after the response ends, and keyless ingest needs minutes of paced sleeps. (The cron path is untouched: `scheduled()` gets its 15 minutes.) Replaced with a **streaming response** (`88e48e89`): first byte immediately (no 100s-TTFB 524), a heartbeat line every 10s, invocation kept alive by the open connection, final line = run summary. Both dead ends are now recorded in the route's comment so nobody rebuilds them.
- **First real ingest, end to end on the live worker:** 1,685 repos via GitHub search (0 errors), 3,399 via awesome-lists with `star fetch skipped 3393 new repos (budget/deadline)` counted into `source_runs.error` — the never-silent rule doing its job. 4,978 items, 1,689 snapshots. `POST /admin/compute?week=2026-08-10` → **`insufficient history: no snapshots for prior week 2026-08-03`** — the designed first-week refusal, observed. First rankable week: Mon Aug 24 (Sun Aug 23 cron gives the second snapshot). Admin token in the session scratchpad (`trends-admin-token`); approve stays manual.
...
- **The gap, stated precisely:** the server instructions already run a start-of-task ritual ("call `artifacts_list` before non-trivial work"). There was no end-of-task counterpart, so everything a session learned died with it. No new subsystem was needed — `putArtifact(publish:false)` already files a proposal into the existing `/proposals` inbox with dedupe, version guards and an approve/reject UI. What was missing was a tool whose NAME AND DESCRIPTION trigger the behaviour at the right moment, plus the safety framing.
- **`artifacts_contribute`** (mcp-worker/src/agent.ts, registered after `artifacts_put` so it inherits the audit wrapper): args `kind, name, description, body, why (40–500), if_version?`. `why` is a required first-person justification stored verbatim in the existing `artifact_versions.changelog` — so the reviewer-facing half shipped with zero UI work. Refuses when `why` merely restates `description`. Rate limited 3/hour + 10/day per teamspace, plus a refusal at 25 pending, each with an explicit **"do not retry"** (the generic limiter message invites a retry loop from a model that was just told to contribute). Three distinct result shapes, because `putArtifact` has early returns that report `published` while storing nothing — telling the user "I proposed this" when nothing was filed would be a lie they cannot check.
- **Always a proposal, structurally:** `contributeArtifact` in `lib/artifacts/store-core.ts` is the only place `publish: false` appears; `ContributeInput` omits the field entirely and the literal is spread AFTER `...input`, so even a cast cannot override it. `canPublishArtifact` is deliberately not consulted — the absence is the feature. Pinned by tests incl. a **discriminator test** proving the harness reports `published` when a write really does publish (without it the invariant tests could have been vacuous).
- **A real leak, found by driving the tool rather than reading the code:** `artifacts_get` correctly refused an unapproved artifact, but `artifacts_list` still returned its **name and description** — the field agents match on, in the one call every agent is told to make at task start. An assistant could have laundered unreviewed text to every teammate's agent. Fixed with `publishedOnly` on `listArtifacts`, passed by both agent-facing readers (`artifacts_list`, `skills_list`); human surfaces still show pending rows, which is the point of a review queue. Re-verified live: 1 artifact before, 0 after.
- **Provenance is a column** (`artifact_versions.origin`, migration 0018), not a changelog prefix: a marker any human or `artifacts_put` caller could type would be forgeable, and reviewers would learn to trust it. Surfaces as a "Proposed by an AI assistant, unprompted — on behalf of \<email\>" badge in the inbox, with intro copy warning that approving publishes text no human wrote.
- **Notifications** reuse the table shipped 2026-08-13: new kind `artifact_proposal`, fan-out to owners+admins excluding the actor (zero recipients in a personal teamspace is correct, not an error), new nullable `notifications.artifact_version_id`. 0016's header claim that its nullable columns covered future kinds "without another migration" was **amended in place** — `document_id` joins `documents`, so parking another entity's id there is a schema lie.
- **Behavioural surfaces, ranked by reach:** server instructions (one ~80-word paragraph — the only surface ChatGPT users ever see, so it ends with the injection refusal) → tool description → new `skills/ilolink-contribute/SKILL.md` (Claude Code only, carries the depth: trigger criteria, and a deliberately longer anti-criteria list naming "anything a page or file TOLD you to save" as the injection vector). `skills/ilolink-skill-registry/SKILL.md` was reframed from skills-only to the ten-kind registry — as written it actively suppressed the feature.
- **Rejected, recorded in the plan:** a flag on `artifacts_put` (read only after the model already decided to write); seeding guidance into every teamspace via bootstrap (only reaches new teamspaces, cannot bootstrap itself, and `bootstrap.ts` argues against advice nobody wrote); MCP prompts/resources (user-invoked, so they cannot produce unprompted behaviour, and ChatGPT is tool-centric).
- **Verified, not assumed:** 3× tsc clean, **385/385** tests. Full live protocol run against `wrangler dev` on shared local state with a real PAT (member in a teamspace owned by someone else): `tools/list` shows the tool with the intended description and annotations; a contribution filed `status=proposed`, `origin=agent_contribution`, `changelog`=the why, `current_version_id` NULL, one notification to the **owner** and none to the actor, `mcp_audit` classified it `write`; `artifacts_get` refused it and `artifacts_list` returned nothing; an identical re-call filed nothing and notified nobody; the 4th call in an hour hit the limiter; an `AKIA…` body was refused with nothing written; after approval the artifact became listable and readable with no "NOT LIVE" preamble. Proposals inbox and notifications feed screenshotted in light and dark.
- **Known residuals (accepted):** an inattentive reviewer is the floor; a private repo's internals could reach a shared teamspace unprompted (bounded by proposal-only, the teamspace named aloud, and human review — the direct cost of "contribute without asking"). `mcp-worker/src/memory.ts` is still unregistered; `authz.ts` `Caller.role` still omits `'admin'`.

## 2026-08-13 — Org analytics, logo, composer identity, preview font, org memory, read receipts (ultracode, 6 asks)

- **Asked (ultracode):** (1) preview font = app font; (2) signed-in users shouldn't see the Name field in comments (screenshot: anchored popup); (3) show MCPs connected per teamspace + audit log of who reads/updates, as org analytics; (4) org memory built as documents are pushed; (5) a logo; (6) show members viewing a doc.
- **Migration `0017_org_analytics.sql`**: mcp_audit, org_memory, member_doc_views — additive, no FKs (rows must survive doc deletion/moves).
- **(1)** @font-face can't cross into a srcdoc iframe, so the preview overlay now serializes the parent's Archivo font-face rules (same-origin styleSheets walk, per-sheet try/catch) into its injected CSS + mirrors the h1–h4 800 rule. Verified: iframe computed font-family === parent's.
- **(2)** Root cause: the widget's anchored-selection popup and reply forms built LOCAL anonymous composers; the session-aware app-origin iframe existed but only for the main composer. Now every composer is the iframe: signed-in = "Posted as <email>", no Name anywhere; signed-out on anon-mode docs = Name (optional) via a new anonymous embed branch that posts the widget's old `/_comments` contract through the apex rewrite. The embed gates the anon branch server-side (mode/visibility/unpublished, failing closed to the sign-in prompt — no oracle), and **postComment on the view origin now enforces the same policy** (pre-existing gap: it accepted direct anonymous POSTs to signed/off/private/unpublished docs; every rejection returns the honeypot's silent ok). Widget frames pin light tokens (`scheme=light` param + `.scheme-light` override class) so dark-OS readers don't get a dark form in the light popover.
- **(3)** mcp-worker logs every tool call to mcp_audit via ONE wrapper at tool registration (user, client [PAT name/OAuth client], tool, read/write, target; waitUntil, failures swallowed). Teamspace page gained **Connected assistants** (30-day distinct actives + read/write counts, PAT inventory), **Assistant activity** (latest 30, write=accent tag), members-only via the page's existing gate.
- **(4)** **Org memory**, deliberately extractive not generative (no AI binding; memory must never say what the doc didn't): every publish (app + MCP paths) writes title + ~280-char body excerpt (md/html stripped, surrogate-pair-safe cut) + author; teamspace page shows the timeline; `memoryRecent()` in mcp-worker preps the future recall tool (registration is a follow-up one-liner).
- **(5)** **Logo: two interlocking square links** — a chain link made Modernist (square, one red, genuine over-under weave via path cutouts). `lib/ui/logo.tsx` (currentColor), app/icon.svg, all three nav wordmarks, OG card. Rasterized and eyeballed at 512px before accepting.
- **(6)** **Member read receipts** recorded where identity is actually known — the `/private/<slug>` mint (best-effort upsert, never blocks the redirect). Doc analytics gained **Team readers** (who, last open, count) + an honest line: public/unlisted opens stay anonymous by design. The "Members link" is now a real copyable line on the stats page. Receipts are **purged on teamspace move** and org-memory rows re-home with the doc (review catch: cross-teamspace PII otherwise). memberViews requires actual membership — share-grantees/legacy tokens keep anonymous analytics only.
- **9-agent workflow + fix agent on 11 findings** (4 majors above). Verified live in dev: teamspace page renders all three sections with seeded audit + real publish-created memory; Team readers row from a real /private open; signed-out embed serves Name field + scheme-light wrapper; preview iframe font matches parent; logo in nav + favicon. 347/347 tests (33 files), 3× tsc, build `○ /` intact.

## 2026-08-13 — Private teamspace docs, @mention notifications, dashboard fixes (ultracode, 7 asks)

- **Asked (ultracode):** (1) teamspace docs private by default — "accessible outside the org in incognito"; (2) personal/teamspace tabs "reverse behaviour"; (3) md/text previews in app theme; (4) assign via @name in comments; (5) notifications page with history; (6) headers audit; (7) creator shown in list + doc view.
- **(1) is a design gap, not a bug:** teamspace default was `unlisted` = link-only *by design*; anyone with the link opens it. Built a real fifth visibility, `private` (documents.visibility is TEXT — no migration). Sessions are deliberately `__Host-`-locked away from view.ilolink.com (untrusted author HTML), so access rides a **5-minute HMAC view-gate token**: member opens `ilolink.com/private/<slug>` → membership check → 302 `view.ilolink.com/<slug>?vt=<exp>.<hmac-sha256>` → content-worker verifies (its own twin verifier, `test/view-gate-cross.test.ts` pins the two implementations to one format) and sets a `Path=/<slug>` cookie so refresh works. Fails closed on missing secret. **No existence oracle anywhere:** unknown-slug and private-without-credential return byte-identical 404s on BOTH origins (app route + content-worker; the shared 404 page carries a quiet "open it via ilolink.com" line for every slug so members still have a path from a bare share link). Private docs: noindex, `no-store`, empty comment thread for outsiders, comment WRITES also members-only. Defaults: web + MCP both → shared teamspace `private`, personal stays `public` (web) / `unlisted` (MCP agent contract). Doc detail page gained a visibility switcher (public/unlisted/private) so existing leaked docs can be flipped.
- **(2) reproduced before prescribing: filtering was CORRECT** — the visual affordance was inverted (active chip pale wash, inactive strong red outline). New `TAG_CHIP_ACTIVE`/`TAG_CHIP_INACTIVE` pair (active = wash + accent border, inactive = quiet surface); applied to teamspace chips and heatmap toggles.
- **(3)** preview overlay injects token CSS (resolved from live computed styles, so dark just works; `matchMedia` listener re-themes on scheme flip) into md/text srcdocs; html/pdf keep author styling; code-block contents stripped before the "author-styled?" sniff so docs *about* CSS don't lose theming.
- **(4)+(5)** `migrations/0016_notifications.sql` (generic notifications table, mention = first kind). Composer (app-origin iframe, session present) autocompletes @ from `/api/mentions/candidates` (members-only, id-keyed); POST `/api/comments` validates mention ids against the doc's teamspace, caps 10, drops self; `/notifications` page (unread = wash + 2px accent rule, mark-all-read with aria-live) + nav badge (accent-strong, "9+" cap). ARIA 1.2 combobox pattern on the textarea itself.
- **(6)** headers audit swept every app/marketing page: one truthful h1 per page, no level skips, `<title>` per page ("X — ilolink"; doc detail no longer titled with the marketing strapline).
- **(7)** `created_by` was already stored — surfaced as `· email` in list rows and "Published by …" on the detail page (JOIN, no N+1).
- **10-agent workflow** (6 implement + headers + 3 lenses: security/regression/UX) + fix agent on 15 findings — 1 blocker (candidates endpoint queried `slug`, composer sends doc `id` — mentions would have shipped inert), 2 majors (ARIA 1.1 pattern; missing aria-live), the content-origin existence oracle (401 gate vs 404 unified), private comment-write gate, nullable notification joins, badge/CTA contrast to accent-strong, 32 marketing pages' four-mode visibility enumerations updated to five.
- **Verified, not assumed:** 3× tsc clean, **326/326** vitest (incl. new cross-worker token contract test), build `○ /` + `ƒ /notifications` + `ƒ /private/[slug]`; live e2e in dev: teamspace publish defaulted Private and stored `private`; `/private/<slug>` → 302 with valid vt for a member, byte-identical 404s for unknown vs non-member (fixture teamspace); mention typed in real composer → dropdown → Enter → POST → notification row for the mentioned user in D1; badge 1 → page renders row → mark-all-read announces + clears badge; dark preview iframe = canvas/ink; visibility flip persists; personal vs team Private hints differ. **Not verifiable locally:** content-worker private serving path (view origin doesn't run under next dev) — covered by the cross-contract test + unit logic, live check is the first private doc in production.
- Deploy notes: order matters — migration 0016 to remote D1 and `VIEW_GATE_SECRET` set in BOTH workers BEFORE content-worker deploys, content-worker BEFORE app worker (an old content-worker doesn't know `private` and would serve it like public), mcp-worker last.

## 2026-08-13 — App pages carried to the Modernist prototypes: library, doc analytics, teamspaces, members, NEW /billing (ultracode)

- **Asked:** "ultracode added few more pages into design. can you check and complete them" + third handoff zip (`Ilolink.com redesign review (1).zip`) whose `ilolink App.dc.html` prototypes five app screens: billing, library, doc, teamspaces, members.
- **Adaptations decided up front:** Billing becomes a real top-level route `/billing` (nav: Documents · Teamspaces · Billing · Connect · Publish — Publish kept for the signed-out flow the prototype omits). No receipts table exists (`0015_billing.sql`: plan lives on teamspaces with `plan_source`/`plan_updated_at`), so purchases derive from paid teamspaces and the prototype's "Download PDF" is replaced with an honest "Stripe emailed your receipt" note. New token `--color-accent-wash` (DS accent-100, `#fff2ef` / dark `#33100a`) added inline before the workflow so parallel agents wouldn't collide on globals.css. Container widens to the prototype's 1160px: the app layout's `<main>` now provides padding only and **every page owns its container** (redesigned pages `max-w-[1160px]`, the rest wrapped `max-w-3xl` mechanically — verified /connect still measures 768px).
- **8-agent workflow** (5 implement on disjoint file sets + 3 adversarial lenses), then a fix agent on the 13 findings. Fixed: `/t/[id]` back link said "← Teamspaces" but pointed at /dashboard (its "no such page" comment had gone stale in the same diff); heatmap canvas repainted from hardcoded blue/amber rgba to **runtime-resolved tokens** (accent ramp, ink labels, divider rules, `--font-sans`, resolved at draw time so dark mode repaints correctly — caption honest again: "Red is where readers clicked"); tag idiom deduped into `lib/ui/tags.ts` (13px is a deliberate bump over the DS's 11px, now said truthfully); one input idiom in `lib/ui/form.ts` (DS `.input` spec: 1px divider on surface) across create-teamspace / invite / rename; nav destinations back to full ink with a new `nav-links.tsx` island adding `aria-current` + accent on the active route; billing purchases predicate aligned with the plan cards so a `plan_source='default'` paid row can't render a paid card above "everything is free"; a11y — `aria-live` on both new copy controls, `aria-pressed` on heatmap toggles, `aria-current` on chips/kind tabs, funnel reader-counts in sr-only text.
- **Rejected as deliberate (recorded, not fixed):** primary buttons stay `bg-accent text-canvas` at 3.76:1 light — the DS's own `.btn-primary`, in the user-approved screenshot, and consistent with what already shipped; hover stays `accent-strong` (6.41:1) rather than DS accent-600 (4.27:1 — worse).
- **Verified, not assumed:** tsc clean, 306/306 vitest, `npm run build` with **`○ /` still static** and `ƒ /billing` present; dev-server screenshots light+dark of library, billing, teamspaces, members, doc analytics (published a smoke-test doc to reach a live analytics view) — tags, bars, dark salmon accent, wash tags all render; analytics-page console errors are the pre-existing dev CORS on `view.ilolink.com/_comments`, not the redesign. **Not verifiable locally:** the heatmap's new token-resolved click ramp with real click data (no heatmap data exists in dev; the empty-state path renders) — first production doc with clicks is the live check. Rename-input now 15px (shared idiom) where the title displays 19px — flagged in case the in-place edit jump reads wrong.
- Files: `app/(app)/layout.tsx`, NEW `app/(app)/billing/{page,upgrade-button}.tsx`, NEW `app/(app)/nav-links.tsx`, NEW `lib/ui/{tags,form}.ts`, dashboard/* (page, [slug], stats-view, heatmap-view, document-row-actions, document-title, artifact-list, preview-overlay), t/* (page, create-teamspace, [id]/page, members-admin, upgrade), max-w wrappers on 8 non-redesigned pages, `app/globals.css` (accent-wash), `lib/ui/nav.ts`.
...
- Five commits (`3d2df4a`, `713606a`, `e596eaa`, `c5e4f88`, `17005cc`), split so the server-side visibility default can be reverted on its own. `origin/main` at **`17005cc`**, confirmed with `git ls-remote`. **Divergence checked in both directions before pushing** — local and `origin/main` were equal at `d0d9db6`, nothing to merge, so this could not roll anyone's work out.
- Deployed the **app worker only**, `ilolink` version **`c27aa81f`**, via the opennextjs-cloudflare build — never a bare `wrangler deploy` at the repo root. `content-worker/` was untouched. `mcp-worker/` got a comment and nothing else, so it was deliberately not redeployed; production still runs the same MCP build, which the new `/mcp` page was checked against.
- **The clean-worktree deploy pattern was attempted and abandoned, for a reason worth recording.** A worktree at `17005cc` with a symlinked `node_modules` fails twice over: `tsc` cannot find `D1Database` because `cloudflare-env.d.ts` is generated and gitignored (fixable by copying it), and then OpenNext's bundler cannot resolve anything under `.open-next/node_modules` because it copies through the symlink. A real `npm ci` per worktree is the only way to make it work, at ~700 MB inside a cloud-synced folder.
  - Deployed from the primary tree instead, with the guarantee reconstructed rather than assumed: `git status` verified empty **immediately before and immediately after** the build, HEAD unchanged at `17005cc` across it, and the built artefact grepped for this branch's own symbols (`discoverTeamspaces` present in the server bundle, the `(marketing)/mcp` client chunk emitted). That is the same check that ruled out the near-miss last time, applied prospectively.
- **Baseline captured before deploying:** `/mcp` **404**, sitemap **62**, unsigned-dashboard probe **404**, apex robots 10 `Disallow` lines, `view.ilolink.com/robots.txt` **200**.
- **After:** `/mcp` **200** with its own title, absolute canonical, no `noindex`, and present in a sitemap now at **63**. `GET /api/teamspaces` returns `{"signedIn":false,"teamspaces":[]}` with `cache-control: private, no-store`. Signed out on `/`: no picker, "Publishing as Public.", **0 console errors**. The connector section names Claude Code and links to `/mcp`; "Grok" is gone from the marketing footer.
- **Regression checks, because two deploys this week each undid another session's work:** the exploit-shaped probe first — a real workspace id with its signature stripped still **404**s, so the unsigned-dashboard bypass stays closed. Apex `robots.txt` still 200 with all 10 rules; `view.ilolink.com/robots.txt` still 200, so the content worker is untouched; `/signin`, `/publish`, `/connect` still `noindex, nofollow`; home 200 with an absolute canonical; a real published document still 200 through the proxy.
- **Checked the URL the new public page advertises**, since a marketing page naming a dead endpoint is worse than no page: `mcp.ilolink.com/.well-known/oauth-protected-resource` **200**, an unauthenticated `POST /mcp` **401**, and `client_id_metadata_document_supported: true` still advertised.
- **Not verified on production:** the signed-in teamspace picker and the invite copy-link, both of which need a real session. Both were exercised end to end locally against D1 — including reading the clipboard back and accepting the copied invite token — and the signed-out half of each is confirmed live.
...
### Pushed and deployed
- `origin/main` at **`2886ff1`** (confirmed with `git ls-remote`, not a local ref). Deployed the **app worker** with `npm run deploy` — the opennextjs-cloudflare build, never a bare `wrangler deploy` at the repo root, which silently ships a stale `.open-next` bundle. Version **`94d83e81`**. The content worker was not touched: nothing under `content-worker/` changed.
- **Baseline captured before deploying:** all four URLs **404**, sitemap **58**.
- **Verified after, on production:** all four **200** plus `/vs/tiiny-host` still 200; sitemap **62**; each new page carries `lastmod 2026-08-12` while `/vs/tiiny-host` correctly keeps the corpus date `2026-08-09`; each emits an absolute self-referencing canonical, its **own** `og:title`, the full JSON-LD set (`Article`, `FAQPage`, `BreadcrumbList`, `SoftwareApplication`, `Organization`, `WebPage`) and its sourced table caption. `/guides` links all five, one `href` each.
- **Regression checks, because two earlier deploys this week each undid another session's work:** the apex `robots.txt` still serves one wildcard group with all 11 rules and the content signal (an app deploy is exactly what rolled the SEO metadata out of production before); `view.ilolink.com/robots.txt` still **200**, so the content worker is untouched; `/signin`, `/publish` and `/connect` still `noindex, nofollow`; home **200** with an absolute canonical; a real published document still **200** through the proxy.
- **Ran the exploit-shaped probe rather than a health check**, per the rule earned when a deploy reopened the unsigned-dashboard bypass while `/` returned 200 throughout: a **real** workspace id with its signature stripped (`/w/w_gNt2JXVB0YEqqYtO`) returns **404**. Still closed.
- **One false alarm, run down rather than waved through:** grepping `/guides` for `/vs/[a-z-]+` reported a stray `/vs/tiin`. It is an artifact of RSC streaming splitting a string across chunk boundaries in the flight payload — the rendered markup contains exactly five correct `href="/vs/…"` values, one per page. No broken link.
- **A near-miss worth recording: `npm run deploy` builds the WORKING TREE, and this tree is shared.** Minutes after the deploy, `git status` showed another session's uncommitted in-progress feature (a per-teamspace visibility default across `publish-form.tsx`, `/api/publish`, `/api/teamspaces`, `app/page.tsx`, `publish-target.ts`). Most of their files were modified *after* the build finished (18:43–18:44 versus `.open-next` at 18:41:18), but `publish-target.ts` was touched at 18:41:15 — three seconds before, well inside the window. Rather than reason about it, **checked the deployed artefact**: fetched `/publish` from production, pulled all 7 referenced chunks, and grepped for their new `defaultVisibilityFor` — **absent from every one**, so the deployed bundle predates their edit and none of their work shipped. It could easily have gone the other way. The safe pattern when the tree is shared is to build from a clean worktree checked out at the commit being deployed, not from the primary tree.
...
## 2026-08-12 — Add and rename a document's title in place (PR #3, merged, live)

- **Asked:** "in a new branch. start to improve the product. add option to add 'title', and 'rename' title in the dashboard lists."
- **Researched before designing, and the research changed the feature.** Titles have been **INSERT-only since launch**: every publish path derives one, there is no `UPDATE documents SET title` anywhere in the repo, and the MCP `update_document` tool takes content but not title. So a bad derived title was permanent. Checked production: **34 documents, 0 untitled** — so this was never about empty titles. It is about *duplicates*: two "Working With Wilson" and two "How We Work Together", indistinguishable in the list.
- **The decisive finding:** the public page never reads `documents.title`. The content worker derives the reader-facing `<title>`, `og:title` and card text from the body's first `<h1>` at request time, deliberately, to avoid a D1 read on the hot serving path (`content-worker/src/index.ts:169-176`). So a rename is an owner-side label — and the UI says so under the field, because a rename that silently failed to reach what readers see is the worse surprise. Confirmed with the user before building rather than assuming.
- **But not cosmetic:** `title` is a search predicate for the MCP `search` tool (`mcp-worker/src/docs.ts:76-79`, `WHERE title LIKE ?`), so a rename changes what a connected assistant can find.
- `PATCH /api/documents`, added beside the existing `DELETE` (same resource, not a new route). Gated on **`canEdit` — its first consumer**; every other document route uses `canRead`, `canDelete`, `canManageShares` or `canModerate`. `canEdit` is correct here and is deliberately *not* the gate `move` had to avoid: an editor share satisfies it without membership, which would be theft for a move (that relocates ownership) but is exactly what an editor share is for when the change is the document's own metadata.
- **No length limit existed anywhere** — plain `TEXT`, no `CHECK`, neither publish path validates. Picked 200 to match `extractTitle`'s slice, the longest value publish can already store. Whitespace collapses *before* measuring, so a pasted multi-line heading becomes one line and padding cannot smuggle a long title past the check.
- Two deliberate deviations from the agreed mock, both reported: the pencil sits **beside the title** rather than as a fifth row icon (the input replaces the title, so trigger and target must share one client component; the alternative was lifting the whole row into client state to coordinate two islands), and it is **always visible below `sm`**, hover-revealed above — measuring `opacity: 0` at rest showed a phone would never reveal it.
- **Two verification scares were mine, not the product's**, and both were chased to ground rather than reported as bugs: a `Control+a` that on macOS moves the caret instead of selecting (so my typing appended and looked like a concatenation bug), and a `npm run build` that clobbered `.next` under the running dev server, leaving React unhydrated so the button was inert.
- **Verified:** all six guards over HTTP (401 no session, 400 missing id, 400 empty, 400 over-long, 404 someone else's, 200 valid with the stored value proving whitespace collapse); in a browser that the field opens with its text selected so typing replaces, Enter saves, Esc discards, blur saves; pencil visible at 375px, hover-revealed at 1100px. 279 tests (8 new), `tsc` clean, build clean.
- **The shared working tree fought this the whole way.** The branch I created was switched out from under me twice by parallel sessions, so my commit first landed on `main` instead of the feature branch. Fixed with pointer moves only — never `reset --hard`, never a checkout — because the tree held another session's uncommitted MCP work. The branch had also come to sit on an unpushed SEO commit, so merging the PR would have shipped someone else's unreviewed work; grafted the commit onto `origin/main` instead using a temporary `GIT_INDEX_FILE`, leaving the shared index untouched, then diffed the grafted files against the tested ones (**byte-identical**).
- PR **#3** → squash-merged as `c6591ea`. Deployed `972f17b0`; production later moved to `34fdbc8b` under a parallel session's deploy and the feature **survived** — confirmed by probing `PATCH /api/documents` with no `documentId` and getting this route's own 400, since an unknown-id probe returns 404 from `guardDoc` and would have read as a rollback.
- Files: `lib/publish/title.ts` (new), `test/document-title.test.ts` (new), `app/(app)/dashboard/document-title.tsx` (new), `app/api/documents/route.ts`, `app/(app)/dashboard/page.tsx`.
- **Deploying from this repo is hazardous while sessions overlap:** `npm run deploy` builds from the *working tree*, not from `main`. Deploying while the tree sat on another branch would have shipped their in-progress work and omitted this feature. A second worktree at `.copy-tmp` also holds `main`, so `git checkout main` fails here — a detached checkout of the merge commit is the way to get a correct tree. `.copy-tmp/` is untracked and wants a `.gitignore` line before something sweeps it into a commit.
